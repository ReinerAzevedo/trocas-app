...
st.set_page_config(
    page_title="Gerenciador de Trocas v4.9", 
    page_icon="🔄", 
    layout="wide"
)
...
if st.session_state['suppliers_dict'] is not None:
    ...
    for supplier, products in suppliers_filtered.items():
        ...
        # HTML DO RECIBO LIMPO E CENTRALIZADO (SEM LOGO)
        html_recibo = f"""<html><head><meta charset='utf-8'><style>
            ...
            h2 {{ margin: 0; font-size: 15px; font-weight: bold; font-family: Arial, sans-serif; text-align: center; text-transform: uppercase; line-height: 1.3; color: #000; border-bottom: 2px solid #000; padding-bottom: 8px; margin-bottom: 12px; }}
